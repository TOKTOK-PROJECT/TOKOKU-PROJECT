...
func (tm *TransaksiMenu) DeleteTransaksi(DeleteTransaksi Transaksi) (bool, error) {
	...
	deleteItemQry, err := tm.DB.Prepare("DELETE FROM item WHERE no_nota=?")
	if err != nil {
	...
	// menjalankan query prepare delete item

	res, err := deleteItemQry.Exec(DeleteTransaksi.NoNota)
	if err != nil {
	...
	if err != nil {
		...
		return false, errors.New("error setelah delete Item")
	}
	...
	// menjalankan query prepare delete transaksi
	result, err := deleteQry.Exec(DeleteTransaksi.NoNota)
	if err != nil {
		log.Println("delete transaksi ", err.Error())
		return false, errors.New("delete transaksi error")
	}
	// Cek jumlah baris yang terpengaruh query diatas
	affeRows, err := result.RowsAffected()
